...
namespace GradPlus.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterJobSeeker(RegisterJobSeekerViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                var result = await UserManager.CreateAsync(user, model.Password);
                //Adding User to role on registration
                if (result.Succeeded)
                {

                    UserManager.AddToRole(user.Id, "Job Seeker");
                    ...
                    Profile_Img=UploadFile(model.ProfileImageFile),
                    ResumeFile= UploadFile(model.ResumeFile),
                    CoverLetterFile=UploadFile(model.CoverLetterFile)                  
                    };
                    userServices.AddJobSeeker(jobSeeker);
        ...
        private string UploadFile(HttpPostedFileBase file)
        {
            ...
            FileName = Helpers.GenerateRandomString(7) + FileExtension;
        ...
        public async Task<ActionResult> RegisterEmployer(RegisterEmployerViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    userServices.AddEmployer(employer);
